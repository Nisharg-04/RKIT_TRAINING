...
SELECT
	AVG(return_date - borrow_date) AS avg_borrow_duration_days
FROM 
	borrowers
WHERE 
...
SELECT
    COUNT(CASE WHEN DATE_PART('month', borrow_date) = 8 THEN 1 END) AS August_Borrows,
    COUNT(CASE WHEN DATE_PART('month', borrow_date) = 9 THEN 1 END) AS September_Borrows
FROM borrowers
WHERE DATE_PART('year', borrow_date) = 2025;
